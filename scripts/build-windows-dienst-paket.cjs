...
async function main() {
  ...
  // Bestehende Produktions-DB sichern, bevor dist neu aufgebaut wird
  const existingDb = path.join(distDir, "prisma", "dev.db");
  const tmpDb = path.join(root, "prisma", "dev.db.build-preserve");
  let preservedDb = false;
  if (fs.existsSync(existingDb)) {
    fs.mkdirSync(path.dirname(tmpDb), { recursive: true });
    fs.copyFileSync(existingDb, tmpDb);
    preservedDb = true;
    console.log("Bestehende Datenbank wird uebernommen (nicht geloescht).");
  }
  ...
  if (preservedDb && fs.existsSync(tmpDb)) {
    ...
    fs.copyFileSync(tmpDb, path.join(destDbDir, "dev.db"));
    fs.unlinkSync(tmpDb);
  }
